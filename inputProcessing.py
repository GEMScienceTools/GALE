...
def cropShakingData(groundShakingData,typeCrop,limit):
    ...
    if typeCrop == 'radius':
        ...
        for iloc in range(len(lon)-1,0,-1):
            distance_x = math.fabs(epicentre[0]-lon[iloc])*111*math.cos(lat[iloc]*math.pi/180)
...
def findClosestData(lon,lat,groundShakingData):
    ...
    dist = numpy.array(((groundShakingData[:,0]-lon)**2+(groundShakingData[:,1]-lat)**2)**0.5)
    ...
    for i in range(len(lines1)):
        ...
        if isfloat(line1[0]):
            ...
            if float(line1[2]) > 0:
                ...
                #sPGA = 0.62
                m = float(line1[2])/100
                mPGA = math.log(m**2/math.sqrt((m**2*(math.exp(sPGA**2)-1))+m**2))
                if len(line1) > 9:
                    sSa03 = float(line2[5])
                    #sSa03 = 0.69
                    m = float(line1[5])/100
                    mSa03 = math.log(m**2/math.sqrt((m**2*(math.exp(sSa03**2)-1))+m**2))
                    
                    sSa10 = float(line2[6])
                    #sSa10 = 0.7
                    m = float(line1[6])/100
                    mSa10 = math.log(m**2/math.sqrt((m**2*(math.exp(sSa10**2)-1))+m**2))
                    
                    sSa30 = float(line2[7])
                    #sSa30 = 0.69
                    m = float(line1[7])/100
                    mSa30 = math.log(m**2/math.sqrt((m**2*(math.exp(sSa30**2)-1))+m**2))
                      
                    Vs30 = float(line1[10])
                    data.append([lon,lat,mPGA,mSa03,mSa10,mSa30,sPGA,sSa03,sSa10,sSa30,Vs30])
                else:
                    Vs30 = float(line1[7])
...
def extract_closest_ground_shaking(exposureLocations,groundShaking):
	...
	closestGroundShaking = numpy.zeros((exposureLocations.shape[0],groundShaking.shape[1]))
