...
def calculate_percentiles_gmfs(percentiles, IMTs, exposureLocations, gmfs):
    ...
    for iper in range(len(percentiles)):
        for i in range(len(IMTs)):
            for iloc in range(noLocations):
                perGMFs[i * noLocations + iloc] = np.percentile(
                    gmfs[i * noLocations + iloc], percentiles[iper])
        save_gmfs_to_xml(
            'gmf_' + str(percentiles[iper]), IMTs, exposureLocations, perGMFs)
...
def parse_nrml_gmf(gmfFile):
    ...
    setGMFs = np.zeros(
        (noIMT * noLocations, len(IMTs) / (noIMT * noLocations)))
