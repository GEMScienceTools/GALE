...
def calculate_distance_matrix(exposureLocations):
	...
	for i in range(noLocations):
		for j in range(noLocations):
			distanceMatrix[i][j]=calculate_distance_2points(exposureLocations[i],exposureLocations[j])
		...
		for j in range(noLocations):
			if i == j:
				...
				if correlationType == 'spatial':
					correlationMatrix[i][j]=math.exp(-3*distanceMatrix[i][j]/b)
...
def calculate_spatial_correlation_matrices(distanceMatrix,IMTs,correlationType):
	...
	for i in range(noIMTs):
		spatialCorrMatrices.append(calculate_single_spatial_correlation_matrix(distanceMatrix,IMTs[i],correlationType))
    ...
    for i in range(noIMT):
        ...
        for j in range(noLocations):
            for k in range(noLocations):
                tempCovMatrix[j][k]=spatialCorrMatrices[i][j][k]*closestGroundShaking[j][2+noIMT+i]*closestGroundShaking[k][2+noIMT+i]
		...
		for j in range(noIMT):
			...
			if i == j:
				...
				if correlationType == 'cross':
					crossCorrMatrix[i][j] = 1-math.cos((math.pi/2)-(0.359+0.163*II*math.log(Tmin/0.189))*math.log(Tmax/Tmin))
...
def generate_random_fields_ground_motion(IMTs,groundShaking,spatialCovMatrices,crossCorrMatrix,siteEffects,noSigmas,noGMFs):
    ...
    for i in range(noIMT):
        LLTrow = []
        for j in range(noIMT):
            LLTrow.append(np.dot(L[i],np.transpose(L[j]))*crossCorrMatrix[i][j])
        for irow in range(len(LLTrow[0])):
            singleLLTrow = np.zeros((int(len(LLTrow)*len(LLTrow[0]))))
            for iL in range(len(LLTrow)):
                singleLLTrow[iL*len(LLTrow[0]):(iL+1)*len(LLTrow[0])] = LLTrow[iL][irow]
    ...
    Z = truncnorm.rvs(-noSigmas, noSigmas, loc=0, scale=1, size=(noLocations*noIMT,noGMFs))
...
def amplifyGMFs(IMTs,Vs30s,gmfs):
    ...
    for i in range(4):
        ...
        for iloc in range(noLocations):
            gmfs[i*noLocations+iloc] = amplify_ground_shaking(T,Vs30s[iloc],gmfs[i*noLocations+iloc]) 
